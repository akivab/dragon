...
class Posts(ndb.Model):
  ...
  def get_small_date(x):
    return str(x.created)[:10].replace('-', '')
  @property
  def nice_date(x):
...
class MainHandler(webapp.RequestHandler):
  def get(self):
    ...
    logged_in = user and user.nickname().lower() == blogsetup.BLOG_USER_ID
...
class AdminHandler(webapp.RequestHandler):
  ...
  def get(self):
    ...
    try:
      ...
      self.redirect('/')
    if user and user.nickname().lower() != blogsetup.BLOG_USER_ID:
      logged_in = False
    ...
    if logged_in:
      ...
      if files:
        newfiles = [f for f in files if f['id'] not in postids or postids[f['id']] != f['modifiedDate']]
    template_values = {
      'logged_in': logged_in,
      'url': url,
      'published': published,
...
class UpdateHandler(webapp.RequestHandler):
  ...
  def updateFile(self, fileId, http):
    ...
    snippet = ''
    pic = '/r/defaults/%d.jpg' % randint(1,4)
    if post:
      post.text = text
...
class BlogHandler(webapp.RequestHandler):
  def findPost(self):
    ...
    if not match:
      ...
      if sdate and stitle:
        return Posts.query(Posts.small_date==sdate,Posts.small_name==stitle).get()
    return None
  def get(self, *args):
  ...
  def post(self, *args):
    ...
    user = users.get_current_user()
    post = Posts.getPostById(fileId)
    if not post or not (user and user.nickname().lower() == blogsetup.BLOG_USER_ID):
      self.error(404)
      return
